...
func updateISCSINode(b iscsiDiskMounter, tp string) error {
	...
	if err != nil {
		...
		klog.InfoS("iscsi: failed to update node with node.session.scan=manual, possible exposure to issue 90982", "result", out)
	}
...
func (util *ISCSIUtil) AttachDisk(b iscsiDiskMounter) (string, error) {
	...
	klog.V(4).InfoS("AttachDisk portal->host map for iscsiIqn", "map", portalHostMap, "iqn", b.Iqn)
	...
	for i := 1; i <= maxAttachAttempts; i++ {
		for _, tp := range bkpPortal {
			...
			if !loggedIn {
				klog.V(4).InfoS("Could not get SCSI host number for portal, will attempt login", "portal", tp)
				...
				klog.V(6).InfoS("AttachDisk portal->host map for iscsiIqn", "map", portalHostMap, "iqn", b.Iqn)
			...
			if iscsiTransport == "" {
				klog.ErrorS(nil, "iscsi: could not find transport name in iface", "iface", b.Iface)
		...
		klog.V(4).InfoS("iscsi: tried all devices for iscsi Iqn, several paths found", "iqn", b.Iqn, "times", i, "pathNumber", len(devicePaths))
		...
		if len(devicePaths) == len(bkpPortal) {
			// We have all paths
			klog.V(4).InfoS("iscsi: all devices for iscsi iqn found", "iqn", b.Iqn)
			break
		}
		if len(devicePaths) >= minMultipathCount && i >= minAttachAttempts {
			...
			klog.V(4).InfoS("Several devices found for iscsi iqn", "pathNumber", len(devicePaths), "iqn", b.Iqn)
			break
		}
	}
	...
	klog.V(5).InfoS("iscsi: AttachDisk", "devicePath", devicePath)
...
func (util *ISCSIUtil) persistISCSI(b iscsiDiskMounter) error {
	klog.V(5).InfoS("iscsi: AttachDisk", "volumeMode", b.volumeMode)
	...
	if err := os.MkdirAll(globalPDPath, 0750); err != nil {
		klog.ErrorS(err, "iscsi: failed to create directory", "path", globalPDPath)
		return err
	}
	...
	if b.volumeMode == v1.PersistentVolumeFilesystem {
		...
		if !notMnt {
			...
			// mount.
			klog.V(4).InfoS("Skipping persistISCSI, the volume is already mounted at path", "path", globalPDPath)
			return nil
		}
	}
...
func deleteDevices(c iscsiDiskUnmounter) error {
	...
	if err != nil {
		klog.ErrorS(err, "iscsi delete devices: could not get devices associated with LUN on target iqn",
	...
	for mpathDevice := range mpathDevices {
		...
		if err != nil {
			klog.ErrorS(err, "Failed to flush multipath device map", "device", mpathDevice)
	...
	for _, deviceName := range deviceNames {
		...
		if err != nil {
			klog.ErrorS(err, "Failed to delete block device", "device", deviceName)
...
func (util *ISCSIUtil) DetachDisk(c iscsiDiskUnmounter, mntPath string) error {
	if pathExists, pathErr := mount.PathExists(mntPath); pathErr != nil {
		...
		klog.InfoS("Warning: Unmount skipped because path does not exist", "mountPath", mntPath)
		return nil
	}
	...
	if !notMnt {
		if err := c.mounter.Unmount(mntPath); err != nil {
			klog.ErrorS(err, "iscsi detach disk: failed to unmount", "mountPath", mntPath)
			return err
		}
	}
	...
	if err = deleteDevices(c); err != nil {
		klog.ErrorS(err, "iscsi detach disk: failed to delete devices")
...
func (util *ISCSIUtil) DetachBlockISCSIDisk(c iscsiDiskUnmapper, mapPath string) error {
	...
	klog.V(5).InfoS("iscsi: get devicePath", "path", devicePath)
...
func ignoreExitCodes(err error, ignoredExitCodes ...int) error {
	...
	for _, code := range ignoredExitCodes {
		if exitError.ExitStatus() == code {
			klog.V(4).InfoS("ignored iscsiadm exit code", "code", code)
			return nil
		}
	}
	return err
...
func execWithLog(b iscsiDiskMounter, cmd string, args ...string) (string, error) {
	...
	if klog.V(5).Enabled() {
		d := time.Since(start)
		klog.V(5).InfoS("Executed command with args over a period time", "command", cmd, "args", args, "time", d, "error", err)
		klog.V(5).InfoS("Executed command with output", "output", string(out))
	}
